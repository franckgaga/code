...
Deterministic state observer based on closed-loop eigenvalue placement. It pre-computes the observer gain with \texttt{place} function from \texttt{ControlSystemsBase.jl}, that implements the method of \citet{placePoles}.
...
Nonlinear model predictive controller with constraints. The objective function supports all the same weights as the linear case. Economic MPCs are also possible by providing an additional custom function that receives the predicted values in argument.
An additional economic term can be added to design economic MPCs
The default optimizer is \texttt{Ipopt.jl}, an open-source interior point method developed by \citet{ipopt}.
