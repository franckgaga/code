...
There is \replaced{seven}{six} \texttt{StateEstimator} concrete types available at the time of writing, all supporting measured $\mathbf{y^m}$ and unmeasured $\mathbf{y^u}$ model outputs. \deleted{The moving horizon estimator will be added soon after this publication.}The following list presents them.
...
\added{%
This approach allows incorporating additional physical information on the process an its disturbances, in the form of constraints on the states and noise estimates:
\begin{alignat}{3}
...
\end{alignat}
and also $\epsilon \ge 0$. The $\mathbf{\hat{X}}$ vectors gather the state estimates over the time window. The $\mathbf{C}$ vectors are non-negative values that specify the softness of the associated bound (equal concern for relaxation). The problem \eqref{eq:J_MHE} is treated as a quadratic program for \texttt{LinModel}, and nonlinear program, for \texttt{NonLinModel}.
}%
