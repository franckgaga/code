...
\end{align}
\end{subequations}
It is worth mentioning that the state update $\mathbf{f}$ and output $\mathbf{h}$ functions must be in pure Julia to design nonlinear MPCs from them, since the implementation rely on \added{the forward-mode} automatic differentiation \added{of \texttt{ForwardDiff.jl} \citep{forwardDiff}}.
