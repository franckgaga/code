...
\end{bmatrix}
\end{equation}
The syntax to construct a linear model with a sample time of $\SI{2}{\second}$ is: \comment{cahnges in the code: added \texttt{setname!} call for y-axis labels in the plots (new feature)}
\begin{minted}{julia}
...
\end{minted}
The \texttt{model} object is used for two purposes : to construct our controller and as a plant simulator to test the design. \added{The \texttt{setname!} function allows customized $y$-axis labels in the plots.}
...
SteadyKalmanFilter estimator and:
 10 prediction steps Hp
  ...
  0 measured disturbances d
\end{minted}
By default, \texttt{LinMPC} controllers use \texttt{OSQP.jl} \citep{osqp} to solve the problem, soft constraints on output predictions $\mathbf{\hat y}$ to ensure feasibility, and a \texttt{SteadyKalmanFilter} to estimate the plant states. The default predictive and control horizons are $H_p = 10 + n_k$ and $H_c = 2$, respectively, where $n_k$ is the number of delays in the linear model.\added{ The $H_c$ value is small by default to reduce the computational burden and the agressiveness \citep{mpcHcAnalysis}.} An attentive reader will also notice that the Kalman filter estimates two additional states compared to the plant model. These are the integrating states for the unmeasured plant disturbances, and they are automatically added at the model output $\mathbf{y}$ by default if observability is preserved.

\added{On a side note, an internal model structure could have been used with \texttt{estim = InternalModel(model)} and \texttt{mpc = setconstraint!(LinMPC(estim); ymin=[45, -Inf])}, as an alternative to state estimator. It was tested on this example and it gave similar results.}\comment{moved footnote in text since the guideline does not allow them.}

Before closing the loop, the actual plant input \added{$\mathbf{u}$ }and measurement \added{$\mathbf{y^m}$ }should initialize the estimates\replaced{ $\mathbf{\hat{x}}$ at the steady-state solution that leads to $\mathbf{\hat{y}^m}(0)=\mathbf{y^m}(0)$. This approach results in a bumpless transfer, especially with estimators in the predictor form. The \texttt{initstate!} function finds this solution for \texttt{LinModel}.}{and measurements should initialize the estimates with \texttt{initstate!} to ensure a bumpless transfer. } \comment{There is multiple steady-state for a given $\mathbf{u}$ value because of the integrating states. There is however only one steady-state solution that leads to $\mathbf{\hat{y}^m}(0)=\mathbf{y^m}(0)$ (with the default unmeasured disturbance model). We need to solve a linear system for that, see the internal doc \texttt{https://tinyurl.com/mrv37fhh}. This is done by the \texttt{initstate!} function. Note this call is useless in this simple example, since the state estimate already starts at the plant value, that is, zero.} Since the plant is the \deleted{linear }model here, its output will initialize the states. \texttt{LinModel} objects are callable for this purpose. Once done, imposing step changes on the setpoint \texttt{ry} and on a load disturbance \texttt{ul} tests the closed-loop performance of \texttt{mpc}:
\begin{minted}{julia}
...
\begin{figure}[h]
    ...
    \caption{CSTR closed-loop simulation\replaced{ (MPC)}{: $y_1=y_L$, $y_2=y_T$, $u_1=u_c$, and $u_2=u_h$}.}
...
\begin{figure}[h]
    ...
    \caption{CSTR closed-loop simulation with feedforward\replaced{ (MPC)}{: $y_1=y_L$, $y_2=y_T$, $u_1=u_c$, and $u_2=u_h$}.}
...
\end{align}
\end{subequations}
in which $g$ is the gravitational acceleration in \si{\meter\per\second\squared}, $L$, the pendulum length in \si{\meter}, $K$, the friction coefficient at the pivot point in \si{\kilogram\per\second}, and $m$, the mass attached at the end of the pendulum in \si{\kilogram}. \replaced{By default, the \texttt{NonLinModel} constructor assumes continuous-time equations and discretizes them with Runge-Kutta:}{Here, the explicit Euler method discretizes the system to construct a \texttt{NonLinModel}}: \comment{changes in the code: using the new in-place/mutating syntax for NonLinModel, using the new built-in RK4 solver, using setname! to clarify the y-axis labels in the plots}
\vspace*{-5pt}
\begin{minted}{julia}
...
The output function $\mathbf{h}$ converts the $\theta$ angle to degrees. \added{The plant model relies on mutating state-space functions to reduce the memory allocations and the computational burden as well. The more intuitive non-mutating syntax can still be used by providing functions with one less argument, e.g.: \texttt{f(x,u,d)=x+u+d} and \texttt{h(x,d)=x+d}.}
...
\begin{figure}[h]
    ...
    \caption{Pendulum state estimation\deleted{: $y_1 = \theta$ in \si{\degree}, $x_1=\theta$ in \si{\radian}, and $x_2=\omega$ in \si{\radian\per\second}}.}
...
The keyword arguments \texttt{Mwt} and \texttt{Nwt} are the output setpoint tracking and move suppression weights, respectively.\added{ The option \texttt{Cwt=Inf} disables the slack variable $\epsilon$.} An angular setpoint of \SI{180}{\degree} (inverted position) tests \texttt{mpc} performance on the plant:
\begin{minted}{julia}
...
\begin{figure}[h]
    ...
    \caption{Pendulum output setpoint tracking\replaced{ (NMPC)}{: $y_1 = \theta$ in \si{\degree} and $u_1 = \tau$ in \si{\newton\meter}}.}
...
\begin{figure}[h]
    ...
    \caption{Pendulum output disturbance rejection\replaced{ (NMPC)}{: $y_1 = \theta$ in \si{\degree} and $u_1 = \tau$ in \si{\newton\meter}}.}
    \label{fig:plot_NonLinMPC3}
\end{figure}

\replaced{The next section presents an economic predictive controller on the pendulum.}{See the package documentation for more advanced examples like economic MPC of the pendulum.}

\textbf{\comment{The following sections are all new in the manuscript (except conclusion).}}
...
\comment{Should I add the successive linearization MPC example of the online doc \texttt{https://tinyurl.com/493ckw2f} ? Its benchmark would certainly standout in Table 1. There are however two issues.} 

\comment{First, there is no direct equivalent in MATLAB since there is no automatic differentiation (AD). I would need to analytically find the Jacobians of the pendulum model and implement a function that evaluates them at the operating point. It's not an equivalent algorithm however since AD is inherently slightly heavier in computation (the derivation part is done by the computer, not me). I could otherwise use symbolic differentiation with \texttt{https://tinyurl.com/4xvadp3y} ? I would avoid finite difference Jacobians since the closed-loop response would be too different.} 

\comment{Second, the Julia OS solver (OSQP.jl) does not work well for this case study without major modification in the optimization parameters (because of the unstable pole). I would only put AS solver (DAQP) results for this example.}

\subsection{Benchmarks}
\label{sec:benchmarks}

The time required to simulate the previous case studies is a benchmark of MPC performance in terms of computational cost (the cost to simulate the plant being negligible). The case studies are reproduced on MATLAB to compare the performance of the two frameworks. A special care is taken to ensure that that the closed-loop solution in MATLAB is identical to Julia’s counterpart. \comment{(Not-So-Fun Fact: This part was surprisingly long. Do you know that the MPC weight parameters in MATLAB are in fact the square root of the final matrices ? For example, if we chose a move suppression weight of 0.5, the associated matrix will be 0.25 in the objective function. I understand this choice, but I would have saved tons of time if they mentioned that in the doc!)}. The median simulation time of 1000 simulations, for MPC, and 50 simulations, for NMPC and EMPC, provide a performance benchmark robust to outliers. It must be emphasized that the reported values are the total time to simulate the whole scenario, that is $N=75$ time steps for the CSTR, and $N=35$, for the pendulum.
...
Disabling the bridges slightly reduce the overhead when the constraints are natively supported by the solver, which is true for all the case studies here. A laptop with an Intel\textsuperscript{\textregistered} Core\textsuperscript{\texttrademark} i7-1165G7 (8 cores at \SI{4.70}{\giga\hertz}) and a Linux v6.8.0 kernel runs the benchmarks. The scripts are executed on Julia v1.10.3 and MATLAB R2024a. A public repository at \texttt{https://github.com/franckgaga/mpcPackageJulia.tex} provides the code to reproduce the results. 
