...
\comment{Should I also include the successive linearization MPC example of \texttt{https://tinyurl.com/493ckw2f} ? Its benchmark would certainly standout in Table 1. There is however two minor issues.} 
...
It is worth mentioning that the closed-loop responses in Julia and MATLAB are all identical for all the case studies. 

The benchmarks results test four types of quadratic and nonlinear solvers. The operator-splitting (OS) methods are: 
\begin{itemize}
...
Note that all the solvers in Julia are free and open-source, except for \texttt{KNITRO.jl} which requires a license. A laptop with an Intel\textsuperscript{\textregistered} Core\textsuperscript{\texttrademark} i7-1165G7 (8 cores at \SI{4.70}{\giga\hertz}) and a Linux v6.8.0 kernel runs the benchmarks. The repository at \texttt{https://github.com/franckgaga/mpcPackageJulia.tex} provides the code to reproduce the results. The benchmark scripts are executed on Julia v1.10.3 and MATLAB R2024a.

The results are presented in \cref{tab:benchamrks}.
...
Many built-in MATLAB functions are not purely interpreted but contains call to specialized compiled code. This is presumably the case for MATLAB \texttt{fmincon} solvers, the function used by MATLAB NMPC toolset. This probably explain why NMPC and EMPC are only twice as fast as MATLAB counterpart on average. Since the pendulum model is simple, this is not performance bottleneck. It is expected that the speed gap between the two languages would be larger for more complex plant models.
