...
As the motor torque is limited to \num{-1.5} to \SI{1.5}{\newton\meter}, the input constraints are incorporated \replaced{in a nonlinear model predictive controller (NMPC) using \texttt{NonLinMPC}:}{to a \texttt{NonLinMPC} object}:
\begin{minted}{julia}
...
\comment{The following sections are all new (except conclusion).}
...
With the sampling time $T_s$ in s, the prediction horizon $H_p$, the limits defined as $t_0=k$ and $t_{end} = (k + H_p) T_s$, and the left-endpoint rectangle method for the integral, we get:
