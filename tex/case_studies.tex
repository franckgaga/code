...
\subsection{Successive Linearization Model Predictive Control (SLMPC)}
\label{sec:successive_linearization}

The \texttt{setmodel!} method allows online adaptation of a linear plant model. Combined with the automatic linearization of \texttt{linearize}, a successive linearization model predictive controller (SLMPC) can be designed with minimal efforts. First, the default \texttt{LinMPC} solver (\texttt{OSQP.jl}) does not perform as well as \texttt{DAQP.jl} on unstable plant like here \citep{daqp}. The syntax to install it is: 
\begin{minted}{julia}
using Pkg; Pkg.add(["JuMP","DAQP"]) # install JuMP and DAQP
using JuMP, DAQP
optim = JuMP.Model(DAQP.Optimizer, add_bridges=false)
\end{minted}
The bridges are a concept introduced by \texttt{JuMP.jl} to convert the constraints into a form that is natively supported by a specific solver (e.g.: from inequality to equality constraints). Disabling them slightly reduce the overhead when it is not required, like here. Also, the \texttt{SteadyKalmanFilter} does not support \texttt{setmodel!}, so we need to use the time-varying \texttt{KalmanFilter} instead:
\begin{minted}{julia}
...
estimator and:
 20 prediction steps Hp
  ...
  0 measured disturbances d
\end{minted}
A custom simulations function that successively linearizes the pendulum model and calls \texttt{setmodel!} on the \texttt{LinMPC} instance is required: 
\begin{minted}{julia}
...
\end{minted}
A new model is set after solving the optimization problem and before updating the state estimate with \texttt{updatestate!} (when both $\mathbf{u}(k)$ and $\mathbf{\hat{x}}_{k-1}(k)$ is available, for the linearization). As shown in \cref{fig:plot_SuccLinMPC1} and \cref{fig:plot_SuccLinMPC2}, the SLMPC closed-loop response is similar to the nonlinear MPC, both for the 180° setpoint:
\begin{minted}{julia}
...
plot(res3_yd)
\end{minted}
but the next section presents the interesting result: the solving time is drastically lower.
