...
\begin{table}
    \centering
    \caption{Julia and MATLAB Benchmarks on the Case Studies. OS solvers: \texttt{OSQP} (Julia) and \texttt{admm} (MATLAB), AS solvers: \texttt{DAQP} (Julia) and \texttt{active-set} (MATLAB), IP solvers: \texttt{IPOPT} (Julia) and \texttt{interior-point} (MATLAB), SQ solvers: \texttt{KNITRO} (Julia) and \texttt{sqp} (MATLAB).}
