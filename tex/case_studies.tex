...
function sim_adapt!(mpc, nlmodel, N, ry, 
               plant, x_0, 𝕩̂_0, y_step)
    U, Y, Ry = zeros(1, N), zeros(1, N), zeros(1, N)
    u, 𝕩̂ = [0], 𝕩̂_0
    initstate!(mpc, u, plant())
    setstate!(plant, x_0); setstate!(mpc, 𝕩̂_0)
    linmodel = linearize(nlmodel; u, x=𝕩̂[1:2])
...
\begin{minted}{julia}
x_0 = [0, 0]; 𝕩̂_0 = [0, 0, 0]; ry = [180]; y_step=[0]
...
\begin{minted}{julia}
x_0 = [π, 0]; 𝕩̂_0 = [π, 0, 0]; ry = [180]; y_step=[10]
...
\begin{itemize}
    \item \texttt{OSQP.jl} in Julia \citep{osqp}
    \item \texttt{admm} in MATLAB \citep{admm}
\end{itemize}
For active-set (AS) methods, the optimizers are: 
\begin{itemize}
    \item \texttt{DAQP.jl} in Julia \citep{daqp}
    \item \texttt{active-set} in MATLAB \citep{activeset}
\end{itemize}
For interior-point (IP) methods, the solvers are: 
\begin{itemize}
    \item \texttt{Ipopt.jl} in Julia \citep{ipopt}
    \item \texttt{interior-point} in MATLAB \citep{interiorpoint}
\end{itemize}
And, lastly, the sequential quadratic (SQ) methods are:
\begin{itemize}
    \item \texttt{KNITRO.jl} in Julia \citep{knitro}
    \item \texttt{sqp} in MATLAB \citep{sqp}
\end{itemize}
...
It can be cloned, and by executing the following commands in the git root folder:
\begin{minted}{julia}
